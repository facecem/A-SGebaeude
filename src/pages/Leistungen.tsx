...
import { Flame, Droplets, Thermometer, Leaf, Wind, AlertTriangle, ArrowRight, Snowflake } from "lucide-react";
...
const services = [
  {
    ...
    desc2: "Wir setzen ausschließlich auf bekannte Hersteller wie Vaillant, Viessmann, Buderus, Bosch und Stiebel Eltron.",
  ...
  },
  {
    ...
    title: "Wärmepumpen",
    category: "Wärmepumpe",
    desc1: "Wärmepumpen sind eine zukunftssichere und energieeffiziente Alternative zu fossilen Heizsystemen. Wir beraten, planen und installieren Ihre Wärmepumpe – von der ersten Einschätzung bis zur Inbetriebnahme.",
    desc2: "Wir verbauen ausschließlich Markengeräte von Vaillant, Viessmann, Buderus, Bosch und Stiebel Eltron.",
    tags: ["Luft-Wärmepumpe", "Erdwärmepumpe", "Vaillant", "Viessmann", "Buderus", "Stiebel Eltron"],
  },
  {
    Icon: Droplets,
    id: "sanitaer",
    ...
    >
      <div className="grid grid-cols-1 lg:grid-cols-[240px_1fr] gap-10 items-start">
        ...
        <div>
          ...
          {service.id === "waermepumpe" && (
            <button
              onClick={() => { setLocation("/waermepumpen"); window.scrollTo({ top: 0, behavior: "smooth" }); }}
              data-testid="link-leistungen-waermepumpen-detail"
              className="group inline-flex items-center gap-2 mt-6 text-[#e8621a] font-semibold text-sm uppercase tracking-wide hover:text-[#f07840] transition-colors"
            >
              Mehr zum Thema Wärmepumpe <ArrowRight className="h-4 w-4 group-hover:translate-x-1 transition-transform" />
            </button>
          )}
        </div>
      </div>
...
export default function Leistungen() {
  useSeo({
    title: "Leistungen – Heizung, Sanitär, Wartung & Notdienst",
    description: "Heizungsanlagen, Sanitär, Wärmepumpen, Wartung und 24/7-Notdienst – alles aus einer Hand. Eigene Monteure in Herzogenrath, Aachen, Köln und Umgebung.",
    path: "/leistungen",
  });
  const [, setLocation] = useLocation();
